(ns open-airplane.worker
  "Cloudflare Worker の入口。**この repo で唯一 Request/Response に触る層。**

  ここには判断を置かない —— どのハンドラが答えるかは
  `open-airplane.route/dispatch` が決め、ページの中身は `open-airplane.view`
  ...
  読み手が開く TypeScript（`worker/src/app.ts`）はどの bundle にも入って
  いなかった（docs/adr/0001）。

  `aget` を使うのは `:advanced-optimization` 下で env のキーが潰れないため
  （先例 `listingops.edge.worker` と同じ約束）。"
  (:require [open-airplane.route :as route]
...
(defn- env->map
  ...
  （中継先 URL だけは route/mcp-router-url を通して意図的に出す）。"
  [env]
  (if env
...
(defn- proxy-xrpc
  ...
  `x-etzhayyim-bff` の値だけは `sveltekit-edge-bff` → `cljs-worker` に変えた
  ——上流に自分が何であるかを名乗るヘッダなので、SvelteKit を名乗り続けるのは
  移行後は嘘になる。docs/adr/0001 に記録した。"
  [req env nsid]
  (let [url (route/mcp-router-url (env->map env))]
    (-> (.json req)
        ...
        (.catch (fn [e]
                  ...
                  ;; 想像上の経路ではなく今日の既定の結末である。移行前は
                  ;; upstream fetch が try/catch されておらず SvelteKit の
                  ;; `{"message":"Internal Error"}` 500 に潰れていた（どの層が
                  ;; 失敗したか応答から読めなかった。移行前 README 欠陥 B）。
...
(defn fetch-handler [req env _ctx]
  (let [url (js/URL. (.-url req))
    ...
    (case action
      ...
      :health (json {:ok true :app "open-airplane" :runtime "cljs"
